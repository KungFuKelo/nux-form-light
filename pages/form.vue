    ...
    <ClientOnly>
        <div class=" mb-20">
            ...
            <div class="p-4 w-[40rem] m-auto" v-for="prop, i in namesProps" :key="i">
                ...
                <input v-model="userInput[i]" v-if="(selectedOpt[i] === '-- other --' || optProps[i].length === 0)"
                    type="text" class="p-3 mt-2 w-full border rounded border-black" placeholder="Enter your value">
            </div>
            <div class="w-full flex justify-center mt-8 mb-10">
                <button @click="resultTable" class="bg-blue-600 text-white border rounded-lg p-4 m-auto">Submit</button>
            </div>
        </div>
        <div v-if="isSubmit" class="mt-20 w-full flex justify-center ">
            <div class="w-[40rem] p-4">
                ...
                <div class="p-4 border rounded-lg bg-slate-200 mt-4 flex justify-between items-center">
                    <p class="mb-0">Main Category:</p>
                    <p class="font-medium mb-0">{{ selectedMain }}</p>
                </div>
                <div class="p-6 border rounded-lg bg-slate-200 mt-4 flex justify-between items-center">
                    <p class="mb-0">Sub Category:</p>
                    <p class="font-medium mb-0">{{ selectedSub }}</p>
                </div>
                <div v-for="prop, i in namesProps" :key="i"
                    class="p-6 border rounded-lg bg-slate-200 mt-4 flex justify-between items-center">
                    <p class="mb-0">{{ prop }}: </p>
...
.active {
    ...
    border-bottom: 1px solid rgb(37 99 235)blue;
}
